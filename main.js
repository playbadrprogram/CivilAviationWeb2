...
const sanaaAirport = createAirport("SANAA", 0, 0);
const adenAirport = createAirport("ADEN", 2500, 3000);
const mukallaAirport = createAirport("MUKALLA", -3200, 2800);
...
for (let i = 0; i < 600; i++) {
    ...
    } while (Math.sqrt(x * x + z * z) < 600); // Clear zone around central airport
...
function createAircraft() {
    ...
    const tailWing = new THREE.Mesh(new THREE.BoxGeometry(10, 0.3, 3), material);
    tailWing.position.set(-10, 0, 0);
    ...
    fin.position.set(-10, 3, 0);
    aircraft.add(fin);

    // Engines
    for (let i = -1; i <= 1; i += 2) {
        const engine = new THREE.Mesh(new THREE.CylinderGeometry(1.2, 1.2, 4, 20), new THREE.MeshStandardMaterial({ color: 0x444444 }));
        engine.rotation.z = Math.PI / 2;
        engine.position.set(0, -1, i * 5);
        aircraft.add(engine);
    }

    // Landing Gear
    for (let i = -1; i <= 1; i += 2) {
        const wheel = new THREE.Mesh(new THREE.CylinderGeometry(0.7, 0.7, 0.5, 16), new THREE.MeshStandardMaterial({ color: 0x111111 }));
        wheel.rotation.z = Math.PI / 2;
        wheel.position.set(-2, -1.2, i * 2);
        aircraft.add(wheel);
    }

    const noseWheel = new THREE.Mesh(new THREE.CylinderGeometry(0.6, 0.6, 0.4, 16), new THREE.MeshStandardMaterial({ color: 0x111111 }));
    noseWheel.rotation.z = Math.PI / 2;
    noseWheel.position.set(8, -1.2, 0);
...
aircraft.scale.set(0.25, 0.25, 0.25);
scene.add(aircraft);

// =====================================
// SYSTEMS FLIGHT VARIABLES
// =====================================
let speed = 0;
let fuel = 100;
let money = 10000;
...
// =====================================
// INPUT HANDLERS (KEYBOARD & MOBILE)
// =====================================
...
style.innerHTML = `
#mobilePanel { position: fixed; bottom: 20px; left: 20px; z-index: 1000; }
#mobilePanel button { width: 60px; height: 60px; font-size: 24px; margin: 4px; background: rgba(255,255,255,0.7); border: none; border-radius: 8px; }
#hudPanel { position: fixed; top: 10px; left: 10px; color: white; font-size: 18px; font-family: Arial; background: rgba(0,0,0,0.5); padding: 15px; z-index: 1000; border-radius: 5px; line-height: 1.5; }
#phaseDisplay { position: fixed; top: 10px; left: 50%; transform: translateX(-50%); color: #00ffcc; font-size: 22px; font-family: 'Arial Black', sans-serif; background: rgba(0,0,0,0.6); padding: 8px 20px; border-radius: 4px; z-index: 1000; text-shadow: 0 0 5px #00ffcc; }
`;
...
// =====================================
// UI INTERFACES (HUD, PHASE & MINIMAP)
// =====================================
...
miniMap.style.position = "fixed"; miniMap.style.top = "10px"; miniMap.style.right = "10px";
...
function updateFlight() {
    ...
    speed *= 0.999; // Aerodynamic Drag

    // Steering & Pitch controls
    ...
    // Update Flight Phase State
    if (altitude < 3) {
        flightPhase = "TAXI";
        // Contextual Feature: Refueling system on runways when parked
        if (speed === 0 && fuel < 100) {
...
function updateHUD() {
    hud.innerHTML = `
        <b>Speed:</b> ${Math.round(speed * 1000)} kn<br>
        <b>Altitude:</b> ${Math.round(altitude)} ft<br>
        <b>Fuel:</b> ${Math.round(fuel)} %<br>
        <b>Money:</b> $${money}
    `;
    phaseDiv.innerText = `PHASE: ${flightPhase}`;
}
...
function createAIAircraft(startX, startZ, targetX, targetZ) {
    const ai = createAircraft();
    ai.scale.set(0.8, 0.8, 0.8);
    ai.position.set(startX, 300, startZ);
    scene.add(ai);
    aiAircrafts.push({ mesh: ai, target: new THREE.Vector3(targetX, 300, targetZ) });
}
...
// =====================================
// MISSION MATRIX SYSTEM
// =====================================
let currentMission = { name: "Fly To Aden", target: adenAirport.position, reward: 5000, completed: false };
...
function updateMission() {
    ...
    if (distance < 150 && altitude < 20 && speed < 0.15 && !currentMission.completed) {
        ...
        currentMission = { name: "Fly To Mukalla", target: mukallaAirport.position, reward: 7000, completed: false };
    }
}
...
function checkCrash() {
    if (altitude <= 2 && verticalSpeed < -0.3) {
        alert("Aircraft Crashed due to hard landing!");
...
function animate() {
    ...
    // Structural building collision checks
    ...
    // Mid-air traffic collision vectors
    for (const ai of aiAircrafts) {
